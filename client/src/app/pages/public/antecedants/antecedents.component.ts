...
export class AntecedentsComponent implements OnInit {
  ...
  public pathologyAnalysis: Array<Analyse>;
  ...
  processAntecedants(): void {
    if (this.patientData && this.pathologyData) {
      this.pathologyAnalysis = this.processService.processPathologies(this.patientData, this.pathologyData);
      this.filterAnalysis();
    }
  }

  filterAnalysis(): void {
    if (this.pathologyAnalysis.length && this.form.value.search !== '') {
      this.filteredAnalysis = this.pathologyAnalysis.filter(patho =>
        patho.name.toLowerCase().includes(this.form.value.search.toLowerCase())
      );
    } else {
      this.filteredAnalysis = this.pathologyAnalysis;
    }
