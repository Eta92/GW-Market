...
@Component({
  selector: 'app-antecedants-ctg',
  templateUrl: './antecedants-ctg.component.html',
  styleUrls: ['./antecedants-ctg.component.scss']
})
export class AntecedentsContingencyComponent implements OnInit {
  ...
  processAntecedants(): void {
    if (this.patientData && this.drugData && this.pathologyData) {
      this.pathologyContingencies = this.processService.processPathologyContingency(
        this.patientData,
        this.drugData,
        this.pathologyData
      );
